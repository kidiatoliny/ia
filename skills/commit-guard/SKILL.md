...
4. Comment-policy scan             — MANDATORY programmatic run:
                                     `bash ~/.claude/skills/commit-guard/scripts/scan-comments.sh`
                                     Exit 1 ⇒ violations on stdout. ALWAYS BLOCK
                                     when this exits non-zero. The only way to
                                     proceed is the comment-skip authorization
                                     protocol below (`cg: <token> reason: <text>`).
...
### Comment-policy violations — stricter authorization

When the comment-policy scan (step 4) reports violations, the standard
`cg: <token>` is NOT sufficient. The user must include an explicit
**reason** explaining why each comment must stay. Required format:

```
cg: <token> reason: <one short sentence per file:line>
```

Examples of valid reasons:

- `reason: bitcast note is a non-obvious mechanism the reader can't infer from the signature`
- `reason: SPDX header required by license audit, scan didn't match because format is non-standard`
- `reason: this file is auto-generated by protoc, banner is part of generator template`

The skill MUST surface each violating line in the pending report so the
user can write a per-line reason. A bare `cg: <token>` reply on a
comment-policy block is treated as a "no, fix the comments" — skill
re-prompts the user that comments must be removed or a reason supplied.

The reason is captured in the final commit-guard summary and persisted
to the commit body as a `Comment-skip:` trailer so the exception is
