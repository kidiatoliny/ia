...
- In the app, go to the **MCP** tab in the Agents sidebar, add `https://mcp.linear.app` as a server, and click **Connect** to authenticate via OAuth
...
If any MCP call fails because Linear MCP is not connected, instruct the user to:

1. Open the **MCP** tab in the Agents sidebar
2. Click **Add Server**, enter name `Linear` and URL `https://mcp.linear.app`
3. Click **Connect** next to the Linear server and complete OAuth in the browser
4. Ensure the toggle is enabled, then retry the request
