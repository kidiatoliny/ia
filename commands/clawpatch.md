...
1. **Create GitHub issue first** (always, regardless of Linear choice):
   ...
   - Create the labels in the repo if missing before the first issue.
...
2. **If Linear sync = yes**, create Linear issue referencing GitHub URL:
   ...
   - `mcp__linear__save_issue` with `team`, `project`, `title` (bare title, no `<severity>:` prefix), `description` (same body shape as above — no metadata header), `priority` (from severity mapping), `assignee: "me"` (always assign to the invoking user), `labels: ["<category>"]` (no `clawpatch` label).
